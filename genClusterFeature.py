...
def create_feature_vectors(data, dict, num):
    """
    ...
    for instance in data:
        ...
        feature_vector = [0 for i in range(num+1)]
        intermediate_text = instance[4]
        tokens = intermediate_text.split()
        for token in tokens:
            index = dict[token.lower()]
...
def generate_arff_file(feature_vectors, num, out_path):
    """
...
if __name__ == "__main__":
    ...
    [dict, num] = getCluster()
    print num
    feature_vectors = create_feature_vectors(data, dict, num)
    generate_arff_file(feature_vectors[:6000], num, "trainCluster.arff")
    generate_arff_file(feature_vectors[6000:], num, "testCluster.arff")
