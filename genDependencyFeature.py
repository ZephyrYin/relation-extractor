...
def parse_data(train_data, test_data):
    ...
    train = D.readFromFile('trainDependency.txt')
    test = D.readFromFile('testDependency.txt')
    ...
    for fp in [train_data, test_data]:
        with open(fp) as f:
            for line in f:
                ...
                shortName = person.replace(' ','').lower()
                shortInstitution = institution.replace(' ','').lower()
                for tuple in allData[cnt]:
                    if tuple[1].lower() == shortName or tuple[1].lower() ==shortInstitution:
                        dict[tuple[2].lower()] = 0
                    elif tuple[2].lower() == shortName or tuple[2].lower() ==shortInstitution:
                        dict[tuple[1].lower()] = 0
...
if __name__ == "__main__":
    ...
    generate_arff_file(feature_vectors[:6000], all_tokens, "trainDependency.arff")
    generate_arff_file(feature_vectors[6000:], all_tokens, "testDependency.arff")
